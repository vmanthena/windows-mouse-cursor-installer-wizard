    ...
    public partial class Form1 : Form
    {
        ...
        private static List<CursorFile> GetCursors()
        {
            ...
            foreach (var regcur in getCursorRegList())
            {
                try
                {
                    var file = new FileInfo(regcur.Value);
        ...
        private static string GetEntryName(string name, string sourceFolder, string prefix, bool includeBaseName)
        {
            if (includeBaseName)
            ...
            if (length > 0 && sourceFolder != null && sourceFolder[length - 1] != Path.DirectorySeparatorChar && sourceFolder[length - 1] != Path.AltDirectorySeparatorChar)
                length++;
            var result = !string.IsNullOrWhiteSpace(prefix) ? $"{prefix}\\{name.Substring(length)}" : name.Substring(length);
            return result;
        }
        ...
        {
            var tempzipFilePath = new FileInfo($"{Path.GetTempPath()}\\{Path.GetTempFileName()}");
            var tempInfPath = new FileInfo($"{Path.GetTempPath()}\\{Path.GetTempFileName()}");
            using (ZipArchive zip = ZipFile.Open(tempzipFilePath.FullName, ZipArchiveMode.Create))
            {
                foreach (var cursorPicture in Controls.OfType<CustomPictureBox>())
                {
                    zip.CreateEntryFromFile(
                        cursorPicture.CursorPath,
                        GetEntryName(cursorPicture.CursorPath, path, string.Empty, false),
                        CompressionLevel.SmallestSize);
                }

                using (StreamWriter file = new StreamWriter(tempInfPath.FullName))
                {
            ...
            MessageBox.Show("Successfully created package", "File created", MessageBoxButtons.OK);
            PackageName.Text = "";
            Submit.Enabled = false;
            InitializeCursors();
        }
